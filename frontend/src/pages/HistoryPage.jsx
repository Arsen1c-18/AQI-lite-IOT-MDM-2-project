...
import { CalendarDays, TrendingDown, TrendingUp, Activity, BarChart2, Clock } from 'lucide-react';
...
const StatCard = ({ label, value, unit, icon: Icon, color }) => (
  <div className="glassmorphism rounded-2xl p-5 border border-white/40">
    ...
    <div className="text-3xl font-bold font-mono text-text-primary">
      {value ?? '--'}<span className="text-base text-text-secondary ml-1">{unit}</span>
    </div>
  </div>
);
...
// ── Page ──────────────────────────────────────────────────────────────────────
const HistoryPage = () => {
  const [selectedRange, setSelectedRange] = useState(TIME_RANGES[1]); // default 24h
  const [activeMetric, setActiveMetric]   = useState('all');

  const { historicalData, latestData, loading } = useAQIData(selectedRange.hours);
  ...
  const chartData = useMemo(() => {
    ...
    return historicalData.map(item => ({
      ...
      AQI:      item.final_aqi,
      RawAQI:   item.raw_aqi ?? null,
      PM25:     item.pm25        ? parseFloat(item.pm25.toFixed(1))        : null,
      CO2:      item.co2         ? parseFloat(item.co2.toFixed(0))         : null,
      Temp:     item.temperature ? parseFloat(item.temperature.toFixed(1)) : null,
      Humidity: item.humidity    ? parseFloat(item.humidity.toFixed(1))    : null,
    }));
  }, [historicalData]);
  ...
  const maxAQI = useMemo(
    () => historicalData.length ? Math.max(...historicalData.map(d => d.final_aqi || 0)) : null,
    [historicalData]
  );
  const minAQI = useMemo(
    () => historicalData.length ? Math.min(...historicalData.map(d => d.final_aqi || 0)) : null,
    [historicalData]
  );
      ...
      <Layout>
        ...
        >
          <div className="w-20 h-20 rounded-full bg-green-50 flex items-center justify-center">
            <BarChart2 className="w-10 h-10 text-accent" />
          </div>
          <div>
            <h2 className="text-2xl font-bold text-text-primary mb-2">No History Yet</h2>
            <p className="text-text-secondary max-w-md">
              No sensor readings found for the last {selectedRange.label}.<br />
              Once your ESP32 starts sending data, charts will appear here.
            </p>
          </div>
        </motion.div>
      </Layout>
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="space-y-8 mb-12">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            ...
            <h1 className="text-3xl md:text-4xl font-bold text-text-primary">Air Quality History</h1>
            <p className="text-text-secondary mt-1">Trends in your air quality over time</p>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <StatCard label="Current AQI" value={latestData?.aqi}  unit="" icon={Activity}    color="text-accent"      />
          <StatCard label={`${selectedRange.label} Average`} value={avgAQI} unit="" icon={Activity}    color="text-blue-500"    />
          <StatCard label={`${selectedRange.label} Peak`}    value={maxAQI} unit="" icon={TrendingUp}  color="text-red-500"     />
          <StatCard label={`${selectedRange.label} Best`}    value={minAQI} unit="" icon={TrendingDown} color="text-green-600"  />
        </div>

        {/* Model source badge */}
        <div className="flex items-center gap-2 text-xs text-text-secondary flex-wrap">
          <span className="px-2.5 py-1 rounded-full bg-blue-50 border border-blue-100 text-blue-600 font-semibold">
            Hybrid AQI
          </span>
          = ML-calibrated (Ridge-v1) when available, otherwise CPCB formula
          <span className="ml-2 px-2.5 py-1 rounded-full bg-amber-50 border border-amber-100 text-amber-600 font-semibold">
            Raw AQI
          </span>
          = CPCB formula from PM2.5 breakpoints only
        </div>

        {/* Main AQI + PM2.5 Chart */}
          ...
          <div className="h-[360px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={chartData} margin={{ top: 10, right: 0, left: -20, bottom: 0 }}>
                ...
                <XAxis dataKey="time" axisLine={false} tickLine={false} tick={{ fill: '#4b7a5e', fontSize: 10 }} minTickGap={40} dy={8} />
            ...
            <div className="h-[240px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={chartData} margin={{ top: 5, right: 0, left: -20, bottom: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="rgba(22,163,74,0.08)" />
                  <XAxis dataKey="time" axisLine={false} tickLine={false} tick={{ fill: '#4b7a5e', fontSize: 10 }} minTickGap={40} />
                  <YAxis axisLine={false} tickLine={false} tick={{ fill: '#4b7a5e', fontSize: 11 }} />
                  <Tooltip content={<CustomTooltip />} />
        ...
        {(activeMetric === 'all' || activeMetric === 'TempHum') && (
          <div className="glassmorphism rounded-3xl p-6 md:p-8 border border-green-100">
            <h2 className="text-xl font-semibold text-text-primary mb-6">Temperature & Humidity</h2>
            <div className="h-[240px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={chartData} margin={{ top: 5, right: 0, left: -20, bottom: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="rgba(22,163,74,0.08)" />
                  ...
                  <Tooltip content={<CustomTooltip />} />
                  <Legend wrapperStyle={{ fontSize: '13px', paddingTop: '16px' }} />
                  <Line type="monotone" dataKey="Temp"     name="Temp (°C)"    stroke="#f97316" strokeWidth={2.5} dot={false} activeDot={{ r: 4 }} animationDuration={1200} />
                  <Line type="monotone" dataKey="Humidity" name="Humidity (%)"  stroke="#06b6d4" strokeWidth={2.5} dot={false} activeDot={{ r: 4 }} animationDuration={1200} />
                </LineChart>
              </ResponsiveContainer>
            </div>
          </div>
